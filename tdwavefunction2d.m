classdef tdwavefunction2d < incident
    ...
    methods
        ...
        function self = tdwavefunction2d(T, M, m,tlag,dirx, diry,wavespeed)
...
%             signalp = @(t) 16*cos(16* arg(t)) .*window(t) + sin(16* arg(t)) .* windowp(t);
            ...
            w = 4;
            sigma = 2*sqrt(2/3);
%             signalp = @(t) w*cos(w* arg(t)) .* exp(-arg(t).^2 / sigma^2) - 2 * (arg(t)) .* sin(w* arg(t)) .* exp(- (arg(t)).^2 / sigma^2) / sigma^2 ;
...
%             time = linspace(0, self.T, self.Nt);
            ...
            for n = 1:self.Nt+1
                ...
                dx(:, :, n) = - self.dirx * R^(n-1) * signalp(time(n));
                dy(:, :, n) = - self.diry * R^(n-1) * signalp(time(n));
